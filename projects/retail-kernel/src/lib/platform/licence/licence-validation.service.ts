...
export type LicenceValidationResult =
  | { readonly valid: true; readonly payload: VerifiedLicencePayload }
...
export class LicenceValidationService {
  ...
  async hasValidLicence(): Promise<boolean> {
    ...
    return !!state && isVerifiedLicencePayload(state.verified_payload) &&
      state.verified_payload.profile_id === this.profile.profile_id && !this.isExpired(state.verified_payload);
  }

...
export class StorageHealthService {
  ...
  async requestPersistence(): Promise<PersistenceRequestResult> {
    if (!navigator.storage || typeof navigator.storage.persist !== 'function') { console.info('Persistent storage API is not supported by this browser.'); return 'unsupported'; }
    try { const granted = await navigator.storage.persist(); console.info('navigator.storage.persist() resolved:', granted); return granted ? 'granted' : 'denied'; }
    catch (error: unknown) { console.error('navigator.storage.persist() failed:', error); return 'error'; }
  }
}

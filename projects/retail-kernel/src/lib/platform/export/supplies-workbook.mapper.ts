...
export class SuppliesWorkbookMapper {
  static map(details: readonly SupplyDetail[]): ExcelSheetDefinition {
    ...
    const rows = details.flatMap((detail) => detail.items.map((item) => {
      ...
      return [detail.supply.date, detail.supply.supplier_name, item.product_name, item.package_type_code, item.quantity_received, unitCostUsd, unitCostUsd * Number(snapshot.exchange_rate), item.subtotal_cost / primaryScale, (item.subtotal_cost / primaryScale) * Number(snapshot.exchange_rate)];
    }));
    const totals = details.reduce((sum, detail) => ({ usd: sum.usd + detail.supply.total_cost / 10 ** detail.supply.currency_snapshot.primary_precision, syp: sum.syp + detail.supply.currency_snapshot.secondary_total_cost / 10 ** detail.supply.currency_snapshot.secondary_precision }), { usd: 0, syp: 0 });

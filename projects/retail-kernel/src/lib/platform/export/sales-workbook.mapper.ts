...
export class SalesWorkbookMapper {
  static map(details: readonly SaleDetail[]): ExcelSheetDefinition {
    ...
    return { name: 'Sales', columns, rows, summary: ['Summary', null, null, null, null, totals.costUsd, totals.costSyp, totals.profitUsd, totals.profitSyp, totals.amountUsd, totals.amountSyp] };
  }
}

function primary(value: number, snapshot: SaleCurrencySnapshot): number { return value / 10 ** snapshot.primary_precision; }
function secondary(value: number, snapshot: SaleCurrencySnapshot): number { return primary(value, snapshot) * Number(snapshot.exchange_rate); }

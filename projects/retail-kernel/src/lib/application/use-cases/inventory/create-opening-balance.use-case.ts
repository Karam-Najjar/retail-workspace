...
export interface CreateOpeningBalanceInput { readonly productId: string; readonly quantity: number; readonly unitCostCents: number; readonly reason?: string; }
...
export class CreateOpeningBalanceUseCase {
  ...
  async execute(input: CreateOpeningBalanceInput): Promise<InventoryBatch> {
    ...
    const adjustment: InventoryAdjustment = { id: adjustmentId, date: now, type: 'opening_balance', product_id: product.id, quantity_change: input.quantity, unit_cost: (input.unitCostCents / 100).toFixed(2), operator_id: operator.id, operator_name: operator.display_name, reason, created_at: now };
    const batch: Omit<InventoryBatch, 'sequence'> = { id: batchId, product_id: product.id, source_type: 'opening_balance', source_id: adjustmentId, original_quantity: input.quantity, remaining_quantity: input.quantity, original_total_cost: totalCost, remaining_total_cost: totalCost, unit_cost_display: (input.unitCostCents / 100).toFixed(2), created_at: now };
    const movement: InventoryMovement = { id: crypto.randomUUID(), product_id: product.id, type: 'opening_balance', quantity_change: input.quantity, batch_id: batchId, sale_id: null, supply_id: null, adjustment_id: adjustmentId, operator_id: operator.id, operator_name: operator.display_name, reason, created_at: now };
    ...
    const activityLog: ActivityLog<InventoryStockAddedEventPayload> = { id: crypto.randomUUID(), event_code: 'inventory.opening_balance.created', entity_type: 'product', entity_id: product.id, entity_name_snapshot: product.name, payload, operator_id: operator.id, operator_name: operator.display_name, related_sale_id: null, related_supply_id: null, created_at: now };

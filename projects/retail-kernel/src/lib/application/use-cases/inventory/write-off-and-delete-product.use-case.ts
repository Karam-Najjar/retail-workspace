...
export interface WriteOffAndDeleteProductInput { readonly productId: string; readonly reason: string; }
...
export class WriteOffAndDeleteProductUseCase {
  ...
  async execute(input: WriteOffAndDeleteProductInput): Promise<readonly InventoryMovement[]> {
    const reason = input.reason.trim(); if (!reason) throw new Error('A reason is required to write off and delete a product.');
    const product = await this.products.getById(input.productId); if (!product) throw new Error('Product could not be found.'); await this.integrity.assertConsistent(product); if (product.quantity <= 0) throw new Error('Products without stock must be deleted normally.');
    const operator = this.activeOperator.activeOperator(); if (!operator) throw new Error('An active operator is required.');
    const now = new Date(); const adjustmentId = crypto.randomUUID();
    const adjustment: InventoryAdjustment = { id: adjustmentId, date: now, type: 'write_off', product_id: product.id, quantity_change: -product.quantity, unit_cost: null, operator_id: operator.id, operator_name: operator.display_name, reason, created_at: now };
    ...
    const activityLog: ActivityLog<InventoryWriteOffEventPayload> = { id: crypto.randomUUID(), event_code: 'inventory.product.written_off', entity_type: 'product', entity_id: product.id, entity_name_snapshot: product.name, payload, operator_id: operator.id, operator_name: operator.display_name, related_sale_id: null, related_supply_id: null, created_at: now };

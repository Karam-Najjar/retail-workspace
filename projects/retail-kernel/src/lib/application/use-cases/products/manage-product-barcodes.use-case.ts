...
export interface ProductBarcodeInput { readonly id?: string; readonly barcode: string; readonly package_type_code: string; readonly multiplier: number; }
...
export class ManageProductBarcodesUseCase {
  ...
  private toBarcode(productId: string, input: ProductBarcodeInput): ProductBarcode {
    ...
    return { id: input.id ?? crypto.randomUUID(), product_id: productId, barcode, normalized_barcode: normalizeBarcode(barcode), package_type_code: input.package_type_code, multiplier: input.multiplier };
  }
}

...
export class AddCartItemUseCase {
  ...
  private async addScanned(rawBarcode: string): Promise<AddCartItemResult> {
    ...
    const line: Omit<DraftCartItem, 'package_quantity' | 'quantity_base_units'> = {
      product_id: product.id, product_name: product.name, product_barcode_id: barcode.id, barcode: barcode.barcode,
      package_type_code: barcode.package_type_code, multiplier: barcode.multiplier,
      selling_price_per_unit: product.selling_price, entry_method: 'scan',
    };
  ...
  private async addSearched(productId: string): Promise<AddCartItemResult> {
    ...
    if (!product) return { status: 'not_found' };
    const line: Omit<DraftCartItem, 'package_quantity' | 'quantity_base_units'> = {
      product_id: product.id, product_name: product.name, product_barcode_id: null, barcode: null,
      package_type_code: null, multiplier: 1, selling_price_per_unit: product.selling_price, entry_method: 'search',
    };
    return this.cart.add(product, line);
  }
}

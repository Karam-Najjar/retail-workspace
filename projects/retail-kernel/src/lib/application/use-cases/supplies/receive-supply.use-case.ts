...
export class ReceiveSupplyUseCase {
  ...
  async execute(input: ReceiveSupplyInput): Promise<Supply> {
    ...
    for (const line of input.lines) {
      if (!Number.isSafeInteger(line.quantityReceived) || line.quantityReceived <= 0) throw new Error('Package quantity must be a positive whole number.');
      ...
      const item: SupplyItem = {
        id: itemId, supply_id: supplyId, product_id: product.id, product_name: product.name,
        package_type_code: packageType.code, quantity_received: line.quantityReceived,
        multiplier: packageType.multiplier, quantity_base_units: quantityBaseUnits,
      ...
      const batch: Omit<InventoryBatch, 'sequence'> = {
        id: batchId, product_id: product.id, source_type: 'supply', source_id: itemId,
        original_quantity: quantityBaseUnits, remaining_quantity: quantityBaseUnits,
        original_total_cost: subtotalCost, remaining_total_cost: subtotalCost,
        unit_cost_display: item.unit_cost_per_base_display, created_at: input.date,
      };
      const movement: InventoryMovement = {
        id: crypto.randomUUID(), product_id: product.id, type: 'supply', quantity_change: quantityBaseUnits,
        batch_id: batchId, sale_id: null, supply_id: supplyId, adjustment_id: null,
        operator_id: operator.id, operator_name: operator.display_name, reason: null, created_at: input.date,
      };
      entries.push({ product, item, batch, movement });
    }

    const supply: Supply = {
      id: supplyId, date: input.date, supplier_id: supplier.id, supplier_name: supplier.name,
      total_cost: totalCost, operator_id: operator.id, operator_name: operator.display_name,
      currency_snapshot: await this.currency.createSupplySnapshot(totalCost), created_at: createdAt,
    };
    const payload: SupplyReceivedPayload = {
      supply_id: supply.id, supplier_id: supplier.id, item_count: entries.length, total_cost: totalCost,
      lines: entries.map((entry) => ({ supply_item_id: entry.item.id, product_id: entry.product.id, quantity_base_units: entry.item.quantity_base_units, subtotal_cost: entry.item.subtotal_cost, batch_id: entry.batch.id })),
    };
    const activityLog: ActivityLog<SupplyReceivedPayload> = {
      id: crypto.randomUUID(), event_code: 'supply.received', entity_type: 'supply', entity_id: supply.id,
      entity_name_snapshot: supplier.name, payload, operator_id: operator.id, operator_name: operator.display_name,
      related_sale_id: null, related_supply_id: supply.id, created_at: createdAt,
    };

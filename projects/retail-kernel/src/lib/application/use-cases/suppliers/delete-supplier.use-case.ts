...
export class DeleteSupplierUseCase {
  ...
  async execute(supplier: Supplier): Promise<number> {
    ...
    const activityLog: ActivityLog<{ affected_supplies: number }> = {
      id: crypto.randomUUID(), event_code: 'supplier.deleted', entity_type: 'supplier', entity_id: supplier.id,
      entity_name_snapshot: supplier.name, payload: { affected_supplies: affectedSupplies },
      operator_id: operator.id, operator_name: operator.display_name,
      related_sale_id: null, related_supply_id: null, created_at: new Date(),
    };

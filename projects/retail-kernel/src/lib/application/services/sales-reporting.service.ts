...
export class SalesReportingService {
  ...
  async getReport(filter: SaleListFilter = {}): Promise<SalesReport> {
    ...
    const details = (await Promise.all(entries.map((entry) => this.repository.getDetail(entry.sale.id)))).filter((detail): detail is SaleDetail => detail !== undefined);
    return {
      details,
      summary: details.reduce<SalesSummary>((summary, detail) => ({

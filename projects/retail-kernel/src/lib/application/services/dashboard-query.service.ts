...
export class DashboardQueryService {
  ...
  async getSnapshot(now = new Date()): Promise<DashboardSnapshot> {
    ...
    const topProducts = [...grouped.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5).map(([product_id, quantity_base_units]) => ({ product_id, product_name: names.get(product_id) ?? product_id, quantity_base_units }));
    return {
      today_sales_usd: todaySales, today_sales_syp: Math.round(todaySales * rate),
      month_sales_usd: monthSales, month_sales_syp: Math.round(monthSales * rate),
      month_profit_usd: monthProfit, month_profit_syp: Math.round(monthProfit * rate),
      low_stock_products: products.filter((product) => product.quantity <= (settings?.low_stock_threshold ?? 0)).map(({ id, name, quantity }) => ({ id, name, quantity })),
      recent_activity: activity, top_products: topProducts,
    };
  }

  watch(): Observable<DashboardSnapshot> { return liveQuery(() => this.getSnapshot()) as unknown as Observable<DashboardSnapshot>; }
  private parseRate(value: string | undefined): number { const rate = Number(value ?? 1); return Number.isFinite(rate) && rate > 0 ? rate : 1; }
}

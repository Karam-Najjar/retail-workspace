...
export class DatabaseInitializerService {
  ...
  await this.database.transaction("rw", this.database.tables, async () => {
    ...
    await this.database.products.bulkPut(
      products.map(product => ({ ...product, selling_price: product.selling_price * MULTIPLIER }))
    );
    ...
    await this.database.inventoryBatches.bulkPut(
      batches.map(batch => ({
        ...
        unit_cost_display: batch.original_quantity > 0
    ...
    await this.database.supplies.bulkPut(
      supplies.map(supply => ({
        ...
        currency_snapshot: supply.currency_snapshot
          ? { ...supply.currency_snapshot, primary_precision: 4 }
          : supply.currency_snapshot,
      }))
    );
    ...
    await this.database.supplyItems.bulkPut(
      supplyItems.map(item => ({
        ...
        unit_cost_per_base_display: item.quantity_base_units > 0
    ...
    await this.database.sales.bulkPut(
      sales.map(sale => ({
        ...
        currency_snapshot: sale.currency_snapshot
          ? { ...sale.currency_snapshot, primary_precision: 4 }
          : sale.currency_snapshot,
      }))
    );

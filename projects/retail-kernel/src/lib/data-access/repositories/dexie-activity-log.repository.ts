...
export class DexieActivityLogRepository implements ActivityLogRepository {
  ...
  async list(filter: ActivityLogListFilter = {}): Promise<readonly ActivityLog[]> {
    const logs = filter.from && filter.to
    ...
    return logs.filter((entry) => (!filter.eventCode || entry.event_code === filter.eventCode) && (!filter.eventCodes?.length || filter.eventCodes.includes(entry.event_code)));
  }
}

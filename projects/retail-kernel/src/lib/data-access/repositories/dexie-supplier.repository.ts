...
export class DexieSupplierRepository implements SupplierRepository {
  ...
  list(): Promise<readonly Supplier[]> {
    return this.database.suppliers.orderBy("name").toArray();
  }
  ...
  countAffectedSupplies(supplierId: string): Promise<number> {
    return this.database.supplies.where("supplier_id").equals(supplierId).count();
  }
}

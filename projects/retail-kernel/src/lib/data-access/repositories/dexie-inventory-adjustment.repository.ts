...
import { InventoryAdjustmentRepository, NegativeInventoryChange, PositiveInventoryChange } from '../../domain/repository-contracts/inventory-adjustment.repository';
...
export class DexieInventoryAdjustmentRepository implements InventoryAdjustmentRepository {
  ...
  applyPositive(change: PositiveInventoryChange): Promise<InventoryBatch> {
    return this.database.transaction('rw', this.database.products, this.database.inventoryBatches, this.database.inventoryMovements, this.database.inventoryAdjustments, this.database.activity_logs, async () => {
  ...
  writeOffAndDelete(change: NegativeInventoryChange): Promise<readonly InventoryMovement[]> {
    return this.database.transaction('rw', [this.database.products, this.database.productBarcodes, this.database.inventoryBatches, this.database.inventoryMovements, this.database.inventoryAdjustments, this.database.activity_logs], async () => {
      const product = await this.requireConsistentProduct(change.product.id, change.product.quantity);
  ...
  private createMovements(change: NegativeInventoryChange, allocations: ReturnType<typeof allocateFifo>['allocations']): readonly InventoryMovement[] {

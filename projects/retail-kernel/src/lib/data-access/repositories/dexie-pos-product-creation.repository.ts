      ...
      async () => {
        const [existingProduct, category, conflictingBarcode] = await Promise.all([
          ...
          this.database.productBarcodes
            ...
            .equals(records.barcode.normalized_barcode)
            .first(),
        ]);
        ...
        const batches = await this.database.inventoryBatches
          .where('product_id')
          .equals(stockedProduct.id)
          .toArray();
        const storedQuantity = batches.reduce(
          (total, batch) => total + batch.remaining_quantity,
          0,
        );
        if (storedQuantity !== stockedProduct.quantity) {
          console.error(
            `Inventory integrity failure for product ${stockedProduct.id}: cached=${stockedProduct.quantity}, batches=${storedQuantity}`,
          );

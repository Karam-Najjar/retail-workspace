...
export class DualCurrencyInputComponent {
  ...
  constructor() {
    effect(() => {
      ...
      else { this.primaryValue.set(primary); this.secondaryValue.set(secondary); }
    });
  }

  protected updatePrimary(value: string | number | null): void {
    const parsed = this.parse(value); if (parsed === null) return;
    this.lastEdited.set('primary'); this.primaryValue.set(parsed); this.secondaryValue.set(parsed * this.rate());
    this.centsChange.emit(Math.round(parsed * 100));
  }

  protected updateSecondary(value: string | number | null): void {
    const parsed = this.parse(value); const rate = this.rate(); if (parsed === null || rate <= 0) return;
    const primary = parsed / rate;
    this.lastEdited.set('secondary'); this.secondaryValue.set(parsed); this.primaryValue.set(primary);
    this.centsChange.emit(Math.round(primary * 100));
  }

  private parse(value: string | number | null): number | null { const parsed = Number(value); return Number.isFinite(parsed) && parsed >= 0 ? parsed : null; }

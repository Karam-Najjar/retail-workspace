...
export class SupplierListComponent implements OnInit {
  protected readonly facade = inject(SuppliersFacade); private readonly router = inject(Router); private readonly dialog = inject(MatDialog); private readonly operators = inject(ActiveOperatorService);
  ...
  protected readonly columns: readonly DataTableColumn[] = [{ labelKey: 'suppliers.name' }, { labelKey: 'suppliers.phone' }, { labelKey: 'suppliers.address' }, { labelKey: 'common.createdBy' }, { labelKey: 'common.createdAt' }, { labelKey: 'common.lastModifiedBy' }];
  readonly rows = (): readonly DataTableRow[] => this.facade.suppliers().map((supplier) => ({ id: supplier.id, values: [supplier.name, supplier.phone || '—', supplier.address || '—', this.operatorName(supplier.created_by_operator_id), this.dateFormatter.format(supplier.created_at), this.operatorName(supplier.last_modified_by_operator_id)] }));

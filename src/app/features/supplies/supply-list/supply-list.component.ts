...
@Component({ selector: 'app-supply-list', imports: [DataTableComponent, DateRangeFilterComponent, EmptyStateComponent, ExportButtonComponent, FormsModule, MatButtonModule, MatDialogModule, MatFormFieldModule, MatSelectModule, SummaryCardComponent, TranslatePipe], providers: [SuppliesFacade], templateUrl: './supply-list.component.html', styleUrl: './supply-list.component.scss' })
export class SupplyListComponent implements OnInit {
  ...
  private filter(): SupplyListFilter { return { supplierId: this.supplierId || undefined, from: this.range.from, to: this.range.to }; }
  ...
  private fileName(): string { return `supplies-report-${this.datePart(this.range.from)}-to-${this.datePart(this.range.to)}.xlsx`; }

...
export class AddStockFormComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    void this.route;
    void this.dialogData;
    try {
      this.exchangeRate.set(await this.facade.loadFormData());
      this.addLine();
    } catch (error: unknown) {
      this.error.set(error instanceof Error ? error.message : "supplies.errors.load");
    } finally {
      this.ready.set(true);
    }
  }
  ...
  protected addLine(): void {
    this.lines.update(lines => [
      ...lines,
      {
        ...
        productId: "",
        packageTypeCode: this.facade.packageTypes[0]?.code ?? "",
        quantityReceived: 1,
        unitCostEntered: 0,
      },
    ]);
  }
  protected updateLine(index: number, line: SupplyLineDraft): void {
    this.lines.update(lines => lines.map((current, currentIndex) => (currentIndex === index ? line : current)));
  }
  protected removeLine(index: number): void {
    this.lines.update(lines => lines.filter((_, currentIndex) => currentIndex !== index));
  }
  protected totalCost(): number {
    return this.lines().reduce((sum, line) => sum + line.quantityReceived * line.unitCostEntered, 0);
  }
  protected totalBaseUnits(): number {
    return this.lines().reduce(
      (sum, line) => sum + line.quantityReceived * (this.facade.packageTypes.find(type => type.code === line.packageTypeCode)?.multiplier ?? 0),
      0
    );
  }
  protected maxDateValue(): string {
    return this.toDateTimeValue(new Date());
  }
  ...
  protected quickAddSupplier(): void {
    this.dialog
      ...
      .afterClosed()
      .subscribe((supplier: Supplier | undefined) => {
        if (!supplier) return;
        this.facade.addSupplier(supplier);
  ...
  protected close(): void {
    ...
    else void this.router.navigate(["/supplies"]);
  }
  protected async save(): Promise<void> {
    if (this.submitting()) return;
    this.error.set(null);
    this.submitting.set(true);
    const supply = await this.facade.receive({
      ...
      lines: this.lines().map(line => ({
        ...
        unitCostEntered: line.unitCostEntered,
      })),
    });
    this.submitting.set(false);
    if (supply) {
      if (this.dialogRef) this.dialogRef.close(supply);
      else void this.router.navigate(["/supplies", supply.id]);

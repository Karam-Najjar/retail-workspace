...
@Component({ selector: 'app-supply-line-editor', imports: [DualCurrencyInputComponent, FormsModule, MatButtonModule, MatFormFieldModule, MatIconModule, MatInputModule, MatSelectModule, ProductPickerComponent, TranslatePipe], templateUrl: './supply-line-editor.component.html', styleUrl: './supply-line-editor.component.scss' })
export class SupplyLineEditorComponent {
  ...
  protected update(values: Partial<SupplyLineDraft>): void { this.lineChange.emit({ ...this.line(), ...values }); }
  protected async selectProduct(productId: string): Promise<void> { this.update({ productId }); await this.autofill(productId, this.line().packageTypeCode); }
  protected async selectPackage(packageTypeCode: string): Promise<void> { this.update({ packageTypeCode }); await this.autofill(this.line().productId, packageTypeCode); }
  protected multiplier(): number { return this.packageTypes().find((type) => type.code === this.line().packageTypeCode)?.multiplier ?? 0; }
  protected baseUnits(): number { return this.line().quantityReceived * this.multiplier(); }
  protected subtotal(): number { return this.line().quantityReceived * this.line().unitCostEntered; }
  protected baseUnitCost(): string { const multiplier = this.multiplier(); return multiplier ? String(this.line().unitCostEntered / multiplier / 100) : '0'; }

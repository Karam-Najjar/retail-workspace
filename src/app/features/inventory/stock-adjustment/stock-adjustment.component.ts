...
export interface StockAdjustmentDialogData { readonly productId: string; readonly initialType?: StockAdjustmentType; }
export interface StockAdjustmentResult { readonly deleted: boolean; }

@Component({
  selector: 'app-stock-adjustment',
  imports: [FormsModule, MatButtonModule, MatDialogModule, MatFormFieldModule, MatInputModule, MatSelectModule, DualCurrencyInputComponent, TranslatePipe],
...
export class StockAdjustmentComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (!productId) { this.error.set('Product could not be found.'); return; }
    ...
    if (!product) { this.error.set('Product could not be found.'); return; }
  ...
  protected availableTypes(): readonly StockAdjustmentType[] {
    ...
    return this.product()?.quantity === 0
      ? ['opening_balance', 'adjustment_in']
      : ['adjustment_in', 'adjustment_out', 'write_off'];
  }

  protected showsUnitCost(): boolean { return this.type === 'opening_balance' || this.type === 'adjustment_in'; }
  protected isWriteOff(): boolean { return this.type === 'write_off'; }
  protected typeChanged(): void { this.syncTypeValues(); }
  ...
  protected async save(): Promise<void> {
    ...
    try {
      let succeeded = false;
      if (this.type === 'opening_balance') succeeded = await this.facade.createOpeningBalance(product.id, this.quantity, this.unitCostCents, this.reason);
      ...
      if (!succeeded) { this.error.set(this.facade.error()); return; }
  ...
  protected cancel(): void { this.close(false, true); }
  ...
  private close(deleted: boolean, cancelled = false): void {
    if (this.dialogRef) { this.dialogRef.close(cancelled ? undefined : { deleted }); return; }

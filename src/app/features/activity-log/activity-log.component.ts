...
export class ActivityLogComponent implements OnInit {
  ...
  protected readonly activityTypes = ['sales', 'supplies', 'productChanges', 'categoryChanges', 'supplierChanges', 'settingsChanges', 'backupChanges'] as const;
  ...
  protected openEntry(id: string): void { const entry = this.facade.entries().find((item) => item.id === id); if (entry?.related_sale_id) void this.router.navigate(['/sales', entry.related_sale_id]); else if (entry?.related_supply_id) void this.router.navigate(['/supplies', entry.related_supply_id]); }
  private filter(): ActivityLogListFilter { const groups: Record<string, readonly string[]> = { sales: ['sale_completed'], supplies: ['supply.received'], productChanges: ['product_created', 'product.created', 'product.deleted', 'inventory.opening_balance.created', 'inventory.stock.added', 'inventory.stock.removed', 'inventory.product.written_off'], categoryChanges: ['category.deleted'], supplierChanges: ['supplier.deleted'], settingsChanges: ['settings.updated'], backupChanges: ['backup.imported'] }; return { from: this.range.from, to: this.range.to, eventCodes: this.activityType ? groups[this.activityType] : undefined }; }

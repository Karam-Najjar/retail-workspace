  ...
  render(entry: ActivityLog): string {
    ...
    switch (eventCode) {
      ...
      case "inventory.stock.removed":
        return this.text("activityLog.eventDetails.stockRemoved", "Stock removed: {{ quantity }} units", {
          quantity: this.numberValue(payload, "quantity") ?? 0,
        });
      case "inventory.product.written_off":
        return this.text("activityLog.eventDetails.writeOff", "Product {{ name }} written off: {{ quantity }} units", {
          name,
          quantity: this.numberValue(payload, "quantity") ?? 0,
        });
      case "supply.received":
        return this.text("activityLog.eventDetails.supplyReceived", "Supply received: {{ items }} items, {{ total }} total", {
          items: this.numberValue(payload, "item_count") ?? 0,
          total: this.money(this.numberValue(payload, "total_cost") ?? 0),
        });

...
export class ProductDetailComponent implements OnInit {
  ...
  protected readonly barcodes = signal<readonly ProductBarcode[]>([]);
  protected readonly batches = signal<readonly InventoryBatch[]>([]);
  protected readonly movements = signal<readonly InventoryMovement[]>([]);
  ...
  editBarcodes(): void {
    ...
    if (id)
      this.dialog
        .open(BarcodeEditorComponent, { width: "min(52rem, calc(100vw - 2rem))", data: { productId: id } })
        .afterClosed()
        .subscribe(() => void this.reload());
  }
  adjustStock(): void {
    const id = this.product()?.id;
    if (id) this.openAdjustment(id);
  }
  requestDelete(): void {
    const product = this.product();
    if (!product) return;
    if (product.quantity === 0) this.confirmDelete.set(true);
    else this.openAdjustment(product.id, "write_off");
  }
  async delete(): Promise<void> {
    const product = this.product();
    if (product && (await this.facade.delete(product))) this.goBack();
  ...
  private openAdjustment(productId: string, initialType?: "write_off"): void {
    if (this.adjustmentDialogRef) return;
    this.adjustmentDialogRef = this.dialog.open<StockAdjustmentComponent, { productId: string; initialType?: "write_off" }, StockAdjustmentResult>(
      StockAdjustmentComponent,
      { width: "min(42rem, calc(100vw - 2rem))", data: { productId, initialType } }
    );
    this.adjustmentDialogRef.afterClosed().subscribe(result => {
      this.adjustmentDialogRef = null;
      if (result?.deleted) this.goBack();
  ...
  private async reload(): Promise<void> {
    ...
    if (!id) return;
    const [detail, inventory] = await Promise.all([this.facade.get(id), this.facade.getInventory(id)]);
    this.product.set(detail?.product ?? null);
    this.barcodes.set(detail?.barcodes ?? []);
    this.batches.set(inventory?.batches ?? []);
    this.movements.set(inventory?.movements ?? []);

...
@Component({
  ...
  providers: [PosFacade, { provide: SCANNER_GATEWAY, useExisting: HidKeyboardTransport }],
...
export class PosPageComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  async ngOnInit(): Promise<void> {
    if (!(await this.multiTab.claimPos())) {
      this.blockedByOtherTab.set(true);
      return;
    }
  ...
  ngAfterViewInit(): void {
    if (this.route.snapshot.queryParamMap.get("autofocus") === "true") this.focusScanInput();
    else this.focusScanInput();
  }
  ngOnDestroy(): void {
    this.destroyed = true;
    this.multiTab.releasePos();
  ...
  private async handleUnknownBarcode(barcode: string): Promise<void> {
    this.scanner.deactivate();
    try {
      const shouldCreate: unknown = await firstValueFrom(
        this.dialog
          .open(UnknownBarcodeDialogComponent, {
      ...
      const created: unknown = await firstValueFrom(
        this.dialog
          .open(UnknownProductCreateComponent, {
  ...
  private activateScanner(): void {
    if (!this.destroyed && !this.facade.checkingOut())
      this.scanner.activate(barcode => {
        void this.processScan(barcode);
      });
  }
